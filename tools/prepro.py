...
- images:     [{image_id, ref_ids, ann_ids, file_name, width, height, h5_id}]
...
/labels is (M, seq_length) int32 array of encoded labels, zeros padded in back
"""
...
forbidden_att = ['none', 'other', 'sorry', 'pic', 'extreme', 'rightest', 'tie', 
'leftest', 'hard', 'only', 'darkest', 'foremost', 'topmost', 'leftish','utmost', 
'lemon', 'good', 'hot', 'more', 'least', 'less', 'cant', 'only', 'opposite', 
'upright', 'lightest', 'single', 'touching', 'bad', 'main', 'remote', '3pm', 
'same', 'bottom', 'middle']
forbidden_verb = ['none', 'look', 'be', 'see', 'have', 'head', 'show', 'strip', 
'get', 'turn', 'wear', 'reach', 'get', 'cross', 'turn', 'point', 'take', 
'color', 'handle', 'cover', 'blur', 'close', 'say', 'go', 'dude', 'do', 'let', 
'think', 'top', 'head', 'take', 'that', 'say', 'carry', 'man', 'come', 'check', 
'stuff', 'pattern', 'use', 'light', 'follow', 'rest', 'watch', 'make', 'stop', 
'arm', 'try', 'want', 'count', 'lead', 'know', 'mean', 'lap', 'moniter', 'dot', 
'set', 'cant', 'serve', 'surround', 'isnt', 'give', 'click']
forbidden_noun = ['none', 'picture', 'pic', 'screen', 'background', 'camera', 
'edge', 'standing', 'thing', 'holding', 'end', 'view', 'bottom', 'center', 
'row', 'piece']

def build_vocab(refer, params):
    """
    Our vocabulary will add 
    __background__, COCO categories, <UNK>, PAD, BOS, EOS
    """
...
def prepare_json(refer, sentToFinal, ref_to_att_wds, iid_to_ann_ids, params):
    ...
    for image_id, image in refer.Imgs.items():
        ...
        ann_ids = iid_to_ann_ids[image_id]
        images += [{'image_id': image_id, 'height': height, 'width': width, 
                    'file_name': file_name, 'ref_ids': ref_ids, 
...
def build_att_vocab(refer, params, att_types=['r1', 'r2', 'r7']):
    ...
    for sent in sents:
        ...
        for att_type in att_types:
            ...
            if len(att_wds) > 0:
                ref_to_att_wds[ref_id] = \
                    ref_to_att_wds.get(ref_id, []) + att_wds
    ...
    print(f'{len(att2cnt)} attribute words are chosen as vocabulary,' 
          f' which are mentioned {sum(att2cnt.values())} times.')
...
def main(params):
    ...
    # image_id -> ordered ann_ids (corresponding to extracted butd feats)
    iid_to_ann_ids = json.load(open(params['iid_to_ann_ids']))['iid_to_ann_ids']
    iid_to_ann_ids = {int(iid): ann_ids for iid, ann_ids in 
                                            iid_to_ann_ids.items()}
    ...
    refs, images, anns, sentences = prepare_json(
        refer, sentToFinal, ref_to_att_wds, iid_to_ann_ids, params)
    json.dump({'refs': refs, 
               'images': images, 
               'anns': anns, 
               'sentences': sentences, 
               ...
               'att_to_ix' : atoi,
               'att_to_cnt': att2cnt,
               'cat_to_ix': {cat_name: cat_id 
                             for cat_id, cat_name in refer.Cats.items()},
               'label_length': params['max_length'],}, 
               open(osp.join('cache/prepro', dataset+'_'+splitBy, 
                    params['output_json']), 'w'))
    print('%s written.' % \
        osp.join('cache/prepro', params['dataset'], params['output_json']))

    # write h5 file which contains /sentences
    f = h5py.File(osp.join('cache/prepro', 
                           dataset+'_'+splitBy, 
                           params['output_h5']), 'w')
    L = encode_captions(sentences, wtoi, params)
    f.create_dataset("labels", dtype='int32', data=L)
    f.close()
    print('cache/prepro/%s/%s written.' % \
          (params['dataset'], params['output_h5']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--splitBy', default='unc', type=str, 
                        help='unc/google')
    parser.add_argument('--iid_to_ann_ids', default='cache/iid_to_ann_ids.json',
                        help='image_id --> ann_ids (ordered)')
